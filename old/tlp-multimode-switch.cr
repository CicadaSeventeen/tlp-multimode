...
if link_directory?(profile_dir) && File.exists?(conf_realpath) && !link_directory?(conf_realpath)
  # ok
else
  STDERR.puts "No such configure"
  exit 1
end
if File.exists?(tlp_conf)
...
puts ""
puts "Running"
if File.exists?(pre_script_realpath) && !link_directory?(pre_script_realpath)
  begin
    puts "Run Pre Script"
    c_system(pre_script_realpath)
  rescue
    puts "Pre Script Run Error"
  end
end
c_system("tlp false > /dev/null")
c_system("tlp start")
if File.exists?(post_script_realpath) && !link_directory?(post_script_realpath)
  begin
    puts "Run Post Script"
    c_system(post_script_realpath)
  rescue
